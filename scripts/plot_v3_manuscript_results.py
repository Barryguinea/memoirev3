"""Génère les figures du manuscrit à partir des artefacts V3 vérifiés."""
...
from revalidation.campaign import _heldout_start_time, has_informative_heldout_signals
from revalidation_v3.campaign import final_params, inject_profile


RESULTS = ROOT / "data" / "revalidation" / "v3_refined_full"
SLS = ROOT / "data" / "revalidation" / "v3_mcgill_sls"
...
def plot_fusion_comparison() -> None:
    ...
    _save(fig, "v3_fusion_comparison")
...
def plot_scenarios() -> None:
    ...
    _save(fig, "v3_scenario_results")


def plot_sls() -> None:
    data = pd.read_csv(SLS / "mcgill_v3_cohort_all_variants.csv")
    ...
    ax.grid(axis="y", alpha=0.25)
    _save(fig, "v3_sls_notifications")
