...
Ce chapitre décrit les données sur lesquelles le système proposé se base pour l'évaluation,
puis le cadre expérimental correspondant. Il présente le corpus principal IceQube 2023, sa
préparation et ses variables dérivées, avant de détailler les campagnes injectées et la
cohorte IceTag-SLS utilisée.
...
Les données principales utilisées dans le traitement proviennent de la ferme du campus
Macdonald de l'Université McGill, sous forme d'un fichier CSV nommé
\texttt{data/brut.csv}. Ce fichier contient 37\,839 mesures de 28 vaches collectées entre le
1\textsuperscript{er} octobre et le 11 novembre 2023. Les variables sont agrégées à une
résolution nominale de 15 minutes. Le Tableau~\ref{tab:dataset} décrit les
caractéristiques vérifiées des données obtenues.
...
\begin{table}[H]
  ...
  \begin{tabular}{lr}
    ...
    Journées avec mesures & 33 sur 42 \\
...
Le corpus IceQube 2023 compte 564 lignes par vache en médiane, chaque ligne correspondant à
un intervalle de 15 minutes. L'étendue, de 244 à 2\,474 lignes, montre que les durées de
suivi diffèrent fortement d'une vache à l'autre, là où la médiane résume la situation
typique. Aucun doublon vache-temps n'est présent. La période s'étend sur 42 journées, dont 33
seulement portent des mesures~: aucune donnée n'est enregistrée du 7 au 15 octobre 2023, et
l'origine de cette interruption n'est pas documentée dans la source. Cinq journées de la
reprise, les 16, 17, 19, 21 et 22 octobre, portent des mesures sans aucune activité~: les pas,
le \textit{Motion Index} et les transitions y sont nuls pour toutes les vaches observées, et la
posture y est enregistrée comme couchée sur la totalité des intervalles. Une telle immobilité
est incompatible avec le comportement d'une vache laitière, qui se lève plusieurs fois par
jour~; ces journées relèvent donc de l'acquisition et non du comportement. Elles représentent
15,6\,\% des couples vache-jour du corpus et tombent à 98\,\% dans les périodes de référence,
dont elles abaissent le niveau attendu. Les conserver rend les résultats conservateurs~: leur
retrait porterait la détection attribuable de 43,2 à 47,7\,\% et l'IoU20 de 29,5 à 34,1\,\%.

\subsection{Variables brutes du corpus}
Les variables exploitées par les deux branches de détection sont les pas, le
\textit{Motion Index}, le temps couché, le temps debout, les transitions et leur
décomposition en levers et couchers, comme montré dans le
Tableau~\ref{tab:variables_brutes}. Là où le Tableau~\ref{tab:dataset} résume le corpus dans
son ensemble, celui-ci détaille les grandeurs mesurées à chaque intervalle. Ces variables
décrivent la quantité de mouvement et la
...
L'autocorrélation des signaux d'activité confirme que les intervalles successifs ne
constituent pas des observations
indépendantes. Après mise sur une grille régulière et déduplication temporelle par vache, les
36\,879 mesures alignées sur cette grille, sur les 37\,839 lignes du
Tableau~\ref{tab:dataset}, servent à ce calcul et correspondent, à ce grain de 15 minutes, à
environ 10\,000 observations indépendantes pour le \textit{Motion Index}. Le temps
d'autocorrélation intégré y est de l'ordre d'une heure pour les signaux d'activité. Au niveau de décision, les totaux
...
médiane ne se multiplie pas par l'effectif~; la somme des tailles effectives, c'est-à-dire du
nombre d'observations réellement indépendantes une fois la dépendance temporelle prise en
compte, reste néanmoins de l'ordre du millier sur l'ensemble du corpus, soit 1\,151 points pour les pas et 880 pour le
...
inter-vache, soit le rapport de l'écart-type à la moyenne calculé sur les moyennes par vache,
atteint 0,50 pour les pas, 0,40 pour le \textit{Motion Index} et 0,41 pour les
transitions. Cette hétérogénéité empêche de définir un seuil commun à tout le troupeau et justifie le
...
% =====================================================================
Cette section décrit les traitements appliqués aux mesures brutes \textbf{avant la détection}~: le
...
exploitable à l'échelle d'un épisode de plusieurs heures. D'autre part, une résolution plus
large masque les transitions posturales. Une résolution de 15 minutes permet d'avoir un compromis entre stabilité du
signal et granularité suffisante pour la persistance recherchée par les branches HYPO et
INSTABILITÉ.

\subsection{Contrôles d'admissibilité}
Le chargement des données impose des dates interprétables, des identifiants non vides et des variables
...
valeurs informatives pour les pas, le \textit{Motion Index} et les transitions. Ce seuil de
14 jours découle du découpage 60/40 de la période de référence~: il laisse environ 8 jours de
référence et 6 jours de période future, soit la durée minimale compatible avec une agrégation
de 12~h, une persistance de 6~h et un délai réfractaire de 24~h. Ces critères
...
La Figure~\ref{fig:admissibilite} présente la durée des séries pour chaque vache.
Les 11 vaches admissibles sont représentées en vert et
...
mais ses signaux ne sont pas informatifs dans la période future~: du 28 octobre au 10 novembre,
ses pas et ses transitions sont nuls alors que son \textit{Motion Index} garde son niveau
habituel, ce qu'un capteur immobile ne produirait pas. Deux compteurs sur quatre sont donc muets
pendant que l'animal continue de se déplacer, et le critère d'admissibilité l'écarte.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.80\textwidth]{figures/cow_admissibility.png}
  \caption{Durée de série par vache et admissibilité à l'évaluation, seuil de quatorze jours.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    box/.style={rectangle, draw=black!55, rounded corners=2pt, align=center,
      minimum height=3.25cm, minimum width=3.55cm, text width=3.15cm,
    ...
    \node[box, fill=loforange!10, right=0.4cm of split] (inject) {Niveau 1\\HYPO: 44,\\stress: 198\\Niveau 2\\INSTABILITÉ: 132};
    ...
    \node[box, fill=ifblue!7, below=0.8cm of main] (sls) {Niveau 3\\IceTag-SLS 2019\\14 vaches};
  ...
  \end{tikzpicture}
  \caption{Plan d'évaluation en trois niveaux.}
...
dix paramètres autour de la référence, soit les huit valeurs du
Tableau~\ref{tab:config_finale} et les deux seuils minimaux de concordance. Elle évalue une robustesse locale et ne sélectionne
...
score du 12 mars 2019 et les données capteurs strictement antérieures. Une vache doit disposer
d'au moins 20 jours de mesures avant cette fenêtre et d'au moins 70\,\% de couverture dans les
