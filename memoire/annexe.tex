...
\begin{enumerate}
  ...
  \item exécuter la validation et l'ablation du module HYPO~;
...
\subsection{Validation et ablation du module HYPO}
\begin{verbatim}
python scripts/run_hypo_module_validation.py \
  --output-dir data/revalidation/hypo_module
\end{verbatim}
...
python -m revalidation.sensitivity \
  --output-dir data/revalidation/hypo_module/parameter_sensitivity
\end{verbatim}
...
\begin{verbatim}
python -m revalidation_v3.sensitivity \
  --output-dir data/revalidation/v3_refined_full
\end{verbatim}
...
python scripts/benchmark_performance.py \
  ...
  --canonical-json data/revalidation/performance_v3_full_corpus.json
\end{verbatim}
...
python scripts/plot_parameter_sensitivity.py \
  --summary data/revalidation/hypo_module/parameter_sensitivity/\
...
python scripts/plot_v3_manuscript_results.py
...
\section{Artefacts principaux}

Le Tableau~\ref{tab:artefacts_revalidation} liste les artefacts qui soutiennent les résultats.

\begin{longtable}{p{0.43\textwidth}p{0.47\textwidth}}
  \caption{Artefacts qui soutiennent les résultats.}
  \label{tab:artefacts_revalidation}\\
  \toprule
  Fichier & Contenu \\
  \midrule
  \endfirsthead
  \toprule Fichier & Contenu \\ \midrule \endhead
  \path{hypo_module/events_primary.csv} & 44 événements de la validation principale HYPO. \\
  ...
  \path{performance_v3_full_corpus.json} & Médiane et décomposition du temps d'exécution de la chaîne complète. \\
  \path{v3_refined_full/comparison_summary.csv} & Comparaison des fusions et variantes de sensibilité. \\
  \path{v3_refined_full/events_fusion_hierarchical.csv} & 132 évaluations de la fusion hiérarchique exploratoire. \\
  \path{v3_refined_full/events_fusion_hypo_only.csv} & Résultats de la branche HYPO seule. \\
  \path{v3_refined_full/events_fusion_instability_only.csv} & Résultats de la branche INSTABILITÉ seule. \\
  \path{v3_refined_full/events_fusion_or.csv} & Résultats de la fusion OU. \\
  \path{v3_refined_full/events_fusion_sequential_24_72h.csv} & Résultats de la fusion séquentielle. \\
  \path{v3_refined_full/events_parameter_sensitivity_*.csv} & Évaluations des variantes de paramètres. \\
  \path{v3_mcgill_sls/mcgill_v3_cohort_all_variants.csv} & Cohorte SLS et sorties des cinq fusions. \\
  \path{v3_mcgill_sls/mcgill_v3_exclusions.csv} & Motifs d'exclusion et couverture. \\
  \path{v3_mcgill_sls/mcgill_v3_metrics.csv} & AUC, Mann--Whitney et Spearman. \\
  \path{v3_mcgill_sls/mcgill_v3_summary.json} & Protocole et synthèse de l'analyse SLS. \\
  \path{memoire/figures/v3_manual_review.png} & Planche reproductible de l'inspection visuelle ciblée. \\
  \bottomrule
\end{longtable}

Sauf pour la planche d'inspection, les chemins du tableau sont relatifs à
\texttt{data/revalidation/}.

\section{Amplitudes des profils d'injection}
...
communes aux deux campagnes, mais leurs durées diffèrent~: 36 à 60 heures dans la validation
...
\begin{table}[H]
  \centering
  \caption{Profils de la validation principale HYPO.}
...
\section{Invariants des injections}
...
\section{Variabilité inter-vache du fond}
...
Le fichier \texttt{data/revalidation/v3\_artifacts.sha256} contient les empreintes SHA-256
des sorties qui alimentent ce mémoire~: validation attribuable, ablation, analyses
