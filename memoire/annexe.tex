...
% on le passe en italique. Le gras, essaye d'abord, rendait l'intertitre plus
% visible que la section A.x qui le contient, alors que la classe impose le
% romain aux sections ; l'italique est aussi celui des intitules de la
% conclusion. \origsubsubsection est la forme \@startsection que la classe
% enveloppe ; la redefinir ici conserve toute sa logique d'espacement.
\makeatletter
...
\begin{enumerate}
  ...
  \item exécuter le test de stress HYPO à aire d'enveloppe appariée, puis la comparaison exploratoire des fusions~;
...
\subsubsection{Test de stress HYPO à aire d'enveloppe appariée}
\begin{verbatim}
...
Le script bibliographique contrôle les 74 références~: 58 DOI sont confrontés aux métadonnées
Crossref ou DataCite et les 16 références sans DOI à une URL d'autorité préalablement revue.
...
\begin{itemize}
  ...
  \item traiter les intervalles de 15 minutes comme des sujets indépendants.
\end{itemize}
...
Le test de stress comporte 198 événements physiques et 990 lignes après application des cinq
...
\begin{longtable}{p{0.43\textwidth}p{0.47\textwidth}}
  ...
  \path{bibliography_audit.csv} & Audit des 74 références par DOI ou URL d'autorité. \\
  \path{memoire/figures/manual_review.png} & Planche reproductible de l'inspection visuelle ciblée. \\
  \path{hybrid_initial/}, \path{hybrid_refined_smoke/} & Exécutions pilotes sur deux vaches, conservées pour la traçabilité~; aucun résultat rapporté n'en provient. \\
  \path{hybrid_sensitivity/}, \path{hybrid_sensitivity_full/} & Variantes de paramètres de l'extension, résumées au Chapitre~\ref{chapitre:resultats}. \\
  \bottomrule
\end{longtable}
\endgroup
