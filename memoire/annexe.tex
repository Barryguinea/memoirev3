...
\begin{enumerate}
  ...
  \item exécuter le stress test HYPO à dose appariée, puis la comparaison exploratoire des fusions~;
...
\subsubsection{Stress test HYPO à dose appariée}
\begin{verbatim}
...
le réglage du détecteur et impose, pour chaque famille modifiée, la même aire discrète de
perturbation que le profil graduel modéré de 48 heures. Les intervalles supprimés du scénario
de données manquantes sont exclus du calcul de cette aire.
...
\path{data/validation/derived_metrics}. Le script d'audit compare ensuite 348 valeurs
...
\begin{longtable}{p{0.43\textwidth}p{0.47\textwidth}}
  ...
  \path{hypo_stress/events.csv} & Sorties des cinq variantes sur les événements de stress à dose appariée. \\
  ...
  \path{hypo_stress/summary.json} & Protocole, empreinte et contrôle d'intégrité des doses. \\
  ...
  \path{manuscript_number_audit.csv} & Correspondance entre 348 valeurs du manuscrit et leurs artefacts sources. \\
