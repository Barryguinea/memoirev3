...
La première méthode de référence repose sur l'algorithme Isolation Forest, ou IF
\citep{liu2008isolation}. Son principe est que les anomalies sont plus faciles à isoler que
...
scikit-learn \citep{sklearn_api} fournit un comparateur ponctuel
...
La méthode nommée Local Outlier Factor (LOF), \citep{breunig2000lof} estime la
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.6cm}
        ...
        Référence fondatrice          & \citep{liu2008isolation}
            & \citep{breunig2000lof} \\
...
directe des performances publiées, comme le soulignent plusieurs revues
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm}
        ...
        \addlinespace
        \citep{oleary2020accelerometers}
    ...
    \end{tabular}
\end{table}
Une revue des accéléromètres pour la détection de boiterie confirme cette dispersion et
souligne l'absence de standardisation dans les protocoles d'évaluation
\citep{oleary2020accelerometers}.
...
reconstruction de signaux plutôt que vers une baisse d'activité directionnelle,
...
\begin{enumerate}
  ...
  référence, ablation contre IF et LOF, analyse OFAT de 20 variantes et test de stress
