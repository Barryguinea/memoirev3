...
teste sa robustesse à des formes moins favorables à dose appariée, puis examine l'extension
...
44 événements, six formes à trois positions dans le stress test, soit 198 événements, et
douze scénarios par vache à la campagne étendue, soit 132 scénarios. Ces effectifs ne sont
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.68\textwidth}c}
    ...
    Dose appariée sur les intervalles conservés du stress test & Conforme \\
...
% =====================================================================
\section{Stress test HYPO à dose appariée}
% =====================================================================
...
Chaque famille perturbée reçoit une aire d'enveloppe de 36 heures, identique à celle du
...
n'est modifié après le gel du protocole.

\begin{table}[H]
  \centering
  \caption{Réponse de HYPO aux formes moins favorables à dose appariée.}
...
Les tests exacts utilisent la moyenne de chaque vache, et non les 165 lignes comme
observations indépendantes. HYPO conserve un avantage net sur IF et LOF, mais aucun avantage
n'est démontré face au comparateur pédométrique doté de la même machinerie temporelle. De
plus, le contrôle pas-seuls produit un nouveau départ attribuable dans 30,3\,\% des cas,
même s'il n'atteint jamais IoU20. Le stress test renforce donc la robustesse de la détection
temporelle face aux formes moins favorables; il ne démontre ni la spécificité de la cohérence
multivariée ni une validité clinique.
...
bilatéral est non concluant ($p=0{,}40$). Le groupe sans exercice ne contient aucun cas
SLS~$\geq2$, donc aucune AUC interne n'y est estimable. Enfin, le retrait successif d'une
vache positive conserve une AUC globale entre 0,909 et 0,955. Le classement descriptif n'est
donc pas porté par une seule vache, mais sa portée clinique demeure limitée par le petit
effectif et la confusion avec le traitement.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.05\textwidth}p{0.25\textwidth}p{0.42\textwidth}p{0.13\textwidth}}
    ...
    3 & Ablation et sensibilité HYPO
      & Supériorité sur IF/LOF; OFAT de vingt variantes; stress test à dose appariée robuste
        aux formes, sans avantage démontré sur le comparateur pédométrique temporel.
      & Réalisé \\
