...
HYPO, soit un total de 44 événements, six formes à trois positions dans le test de stress, pour
...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.68\textwidth}c}
    ...
    Aire d'enveloppe appariée sur les intervalles conservés du test de stress & Conforme \\
    Trois positions postérieures à la période de référence & Conforme \\
    Exécution propre utilisée pour l'attribution & Conforme \\
    Journées sans activité mesurée recensées & Conforme \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    Ensemble des dégradations graduelles & 33 & 57,6\,\% & 87,9\,\% & 39,4\,\% & 0,183 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Compromis détection-charge de fond, avec les injections en (a) et le fond en (b).}
  \label{fig:detection_background_curve}
\end{figure}

Rapportée à une exploitation, la charge de fond devient concrète~: sur la ferme type de
65~vaches du Tableau~\ref{tab:impact_economique}, elle représente environ 26~notifications
par jour, et près de 32 au 90\textsuperscript{e} percentile. Ce volume dépasse ce qu'une
observation quotidienne peut absorber. Un déploiement ne traiterait donc pas la liste
entière, mais son en-tête~: le classement décrit au Chapitre~\ref{chapitre:interface}
ordonne les vaches par priorité et permet d'arrêter la vérification à un nombre fixé
d'avance. Ce mémoire n'établit pas ce plafond, faute d'étalon clinique qui en mesurerait
le coût en épisodes manqués.
...
panneau, le bandeau d'alerte commence dix heures après l'ouverture de la fenêtre et se
prolonge au-delà de sa fermeture~: seuls les départs absents de l'exécution propre y sont
crédités comme nouveaux. La fenêtre injectée
...
pas de temps. Dans chaque panneau, les quatre
...
courbe contre l'axe. Le temps couché accompagne les trois canaux d'activité parce que la
posture pèse 0,20 dans le score et porte la fragmentation posturale de la branche
INSTABILITÉ. Cette inspection vérifie la cohérence technique des sorties.
...
\begin{landscape}
  ...
  \begin{figure}[H]
    ...
    \caption{Quatre situations de la vache 8081 après le gel des paramètres.}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \textit{Motion Index} seul & 34,1\,\% & 25,0\,\% & 0,135 & 0,491 & 0,63 \\
...
\begin{figure}[H]
  ...
  \caption{Amplitude des écarts sur les vingt variantes OFAT, paramètre par paramètre.}
...
% =====================================================================
\section{Test de stress HYPO à aire d'enveloppe appariée}
% =====================================================================
...
\begin{table}[H]
  \centering
  \caption{Comparaisons appariées sur les cinq formes positives du test de stress.}
...
c'est la seule dimension où sa contribution propre se manifeste dans ce test de stress. IF et LOF
...
Le test de stress établit ainsi une robustesse temporelle positive face aux cinq formes moins
...
Le Tableau~\ref{tab:fusions} compare les cinq stratégies sur 132 scénarios, définies au
Chapitre~\ref{chapitre:systeme}~; la règle OU y notifie dès qu'une branche s'active, sans
exiger de concordance. La colonne
...
de HYPO. Dans la Figure~\ref{fig:fusions}, le panneau~(a) donne la réponse de chaque règle,
le panneau~(b) sa charge de fond, la ligne pointillée celle de la fusion hiérarchique. La
surveillance, issue de la seule branche INSTABILITÉ, y est constante.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/fusion_comparison.png}
  \caption{Comparaison des cinq règles de fusion, avec la réponse en (a) et le fond en (b).}
  \label{fig:fusions}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Réponse de la fusion hiérarchique aux douze scénarios injectés.}
  \label{fig:scenarios}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Notifications des sept jours précédant le score SLS, à raison d'une vache par point.}
  \label{fig:sls}
\end{figure}
...
séquences synthétiques, lecture complémentaire du comportement qui garde HYPO comme résultat
principal. Son apport tient à la séparation des sorties et au gain de couverture, tandis que
l'IoU20 inchangé et la charge de fond accrue définissent les priorités de son
perfectionnement.
