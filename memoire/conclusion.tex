...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.22\textwidth}p{0.34\textwidth}p{0.35\textwidth}}
    ...
    Soutenu mais limité
      & Le mécanisme temporel reste robuste à cinq formes moins favorables à dose appariée.
...
profils. La direction du changement et l'accumulation temporelle sont donc mieux soutenues
...
Le stress test à dose appariée apporte cette comparaison sur cinq formes moins favorables,
trois positions et onze vaches. Sur les 165 événements positifs, HYPO atteint en moyenne
97,0\,\% de couverture attribuable, 71,5\,\% de nouveaux départs et 30,9\,\% à IoU20. Il
conserve un avantage exact sur IF, IF avec persistance et LOF avec persistance
($p=0{,}000488$ pour chaque comparaison). En revanche, sa couverture ne dépasse pas celle du
comparateur pédométrique temporel (différence $-0{,}012$, $p=0{,}844$), et le contrôle
pas-seuls produit un nouveau départ dans 30,3\,\% des cas. Cette campagne renforce donc la
robustesse de l'architecture temporelle aux formes moins favorables, mais ne permet pas
d'attribuer un gain indépendant à la cohérence multivariée.
...
d'une atteinte individuelle. La carte thermique de la Figure~\ref{fig:streamlit_heatmap} rend
...
et demeure donc intra-individuel. Ils ouvriraient un axe orthogonal aux trois niveaux de
...
n'est pas évaluée ici, et son effet sur la couverture attribuable reste à mesurer~: écarter
...
maximal et la surveillance d'instabilité séparent moins bien les groupes. Cette concordance
reste limitée par l'effectif et par le fait que les trois vaches SLS~$\geq2$ appartiennent
...
locomoteurs ou à un parage thérapeutique \citep{paudyal2021lying}. Enfin, le stress test
réexécute la comparaison sur un départ abrupt, des familles désynchronisées, une récupération
asymétrique, du bruit et un bloc manquant. L'aire de perturbation est identique entre les
formes et le comparateur pédométrique est inclus. Il confirme que l'avantage sur IF et LOF
ne dépend pas de la seule forme graduelle, tout en révélant honnêtement que l'apport propre
de la cohérence multivariée n'est pas isolé. Les formes injectées demeurent néanmoins des
hypothèses d'évaluation technique et non des trajectoires cliniques observées.
