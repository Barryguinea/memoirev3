...
\begin{table}[H]
  ...
  \begin{tabular}{p{0.35\textwidth}p{0.55\textwidth}}
    ...
    \texttt{revalidation/campaign.py} & Campagne principale de HYPO après la période de référence. \\
    \texttt{revalidation/ablation.py} & Ablation HYPO contre IF et LOF. \\
    \texttt{revalidation/sensitivity.py} & Analyse OFAT des paramètres HYPO. \\
    ...
    \texttt{revalidation\_v3/profiles.py} & Douze scénarios synthétiques et contrôles. \\
    \texttt{revalidation\_v3/campaign.py} & Injections après la période de référence et métriques attribuables. \\
    \texttt{revalidation\_v3/sensitivity.py} & Comparaison des fusions et sensibilité technique. \\
    \texttt{revalidation\_v3/}\\\texttt{mcgill\_sls\_validation.py} & Cohorte et mesures SLS. \\
...
\path{plot_v3_manuscript_results.py} produit les figures de l'extension. Les scripts
...
devenir une alerte de boiterie.
...
\textbf{Algorithme 1:} Fusion hiérarchique des branches HYPO et INSTABILITÉ (par vache)
...
\citep{breunig2000lof}. Il est exécuté en mode nouveauté dans \path{revalidation/ablation.py},
...
Le module \path{revalidation/campaign.py} sélectionne les vaches admissibles, exécute une
référence propre et injecte les quatre profils de la campagne HYPO. Le module
\path{revalidation/ablation.py} réutilise les mêmes fenêtres pour les cinq variantes. Le
module \path{revalidation_v3/campaign.py} étend ensuite le protocole aux douze scénarios
...
module de \path{revalidation_v3}, qui écrit des fichiers CSV et JSON~; un manifeste SHA-256
