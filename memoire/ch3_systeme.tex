...
système CowAlert, sont d'abord agrégées par intervalles de quinze minutes. Cette étape
...
et INSTABILITÉ, dont les épisodes sont ensuite fusionnés en une notification de vérification
ou en une simple surveillance.
...
% =====================================================================
Chaque série est triée chronologiquement. Les 60\,\% des intervalles admissibles
...
douze heures. Pour les trois familles d'activité, le déficit est défini par
\(
d_j(t)=\max(0,1-r_j(t))
...
somme cumulée unilatérale de \citet{page1954continuous}. Il est défini par
\(C_t=\max(0,C_{t-1}+e_t-0{,}07)\), où \(e_t\) désigne l'évidence accumulée à l'intervalle
...
le cumul en l'absence d'évidence nouvelle, ce qui empêche une accumulation indéfinie.
...
\item \textit{Référence individuelle:}\\
    Pour chaque famille de signaux, soit les pas, le \textit{Motion Index}, les transitions et
    la posture, le détecteur calcule la médiane de la période de référence par créneau horaire,
    puis se replie sur la médiane globale de la vache lorsque ce créneau est trop peu documenté.
...
\item \textit{Pour chaque intervalle $t$ de la période surveillée:}\\
    ...
    conserver le ratio correspondant à la variation la plus marquée\\
