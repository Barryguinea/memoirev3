...
pourcentages décrivent la réponse du logiciel à des scénarios contrôlés; ils ne sont pas des
performances cliniques. Le Tableau~\ref{tab:cles_lecture_resultats} rassemble les clés de
lecture mobilisées tout au long du chapitre.
  ...
  \begin{tabular}{p{0.28\textwidth}p{0.64\textwidth}}
    ...
    IoU20
      & Le recouvrement temporel entre l'épisode attribuable et l'événement atteint au moins
        0,20; il renseigne la localisation, pas la validité clinique. \\
    Charge de fond
      & Les notifications hors fenêtres injectées sur des données non étiquetées; sans
        diagnostic exhaustif, elles ne peuvent pas être appelées faux positifs. \\
...
\section{Intégrité des protocoles}

Onze vaches satisfont les critères de durée et de signaux informatifs. La campagne HYPO
comprend quatre événements indépendants par vache, soit 44 événements. La campagne étendue
comprend douze scénarios par vache, soit 132 événements. Tous commencent après la séparation
chronologique, utilisent des identifiants et des fenêtres physiques distincts et sont
comparés à une exécution propre de la même vache. Les injections ne modifient pas le fichier
brut, les variables restent non négatives et le temps postural total est conservé. Le
Tableau~\ref{tab:protocol_checks} récapitule les contrôles automatiques correspondants.
...
Cette quantité mesure une charge de fond, pas un taux de faux positifs, car les données
propres ne possèdent pas d'annotation clinique exhaustive.
...
\subsection{Ablation contre IF, LOF et comparateur pédométrique}

Les cinq variantes sont évaluées sur les mêmes 44 fenêtres. Ici, le recouvrement désigne
tout épisode attribuable qui croise l'événement; il est donc plus permissif que le critère de
nouveau départ utilisé précédemment. La cinquième variante, le comparateur pédométrique,
restreint le détecteur au seul canal des pas, à la manière d'un podomètre. Le
Tableau~\ref{tab:hypo_ablation} rapporte les métriques attribuables des cinq variantes.

\begin{table}[H]
  \centering
  \caption{Ablation attribuable du module HYPO sur les mêmes événements.}
  \label{tab:hypo_ablation}
  \small
  \begin{tabular}{lrrrr}
    \toprule
    Variante & Recouvrement & IoU20 & IoU moyen & Fond/vache-jour \\
    \midrule
    A. HYPO temporel multivarié & 43,2\,\% & 29,5\,\% & 0,137 & 0,402 \\
    B. IF + règles de persistance & 15,9\,\% & 0,0\,\% & 0,004 & 0,152 \\
    C. IF ponctuel & 13,6\,\% & 0,0\,\% & 0,002 & 7,658 \\
    D. LOF + règles & 15,9\,\% & 0,0\,\% & 0,005 & 0,107 \\
    E. Comparateur pédométrique (pas seuls) & 27,3\,\% & 31,8\,\% & 0,141 & 0,518 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Son recouvrement est également
supérieur à B et D ($p=0{,}04297$) et à C ($p=0{,}01562$). L'avantage est aussi de
...
un IoU quasi nul. Sa faiblesse est ailleurs~: restreint aux pas, il recouvre moins
d'événements (27,3 contre 43,2~\%; tendance en faveur de HYPO, $p=0{,}0625$ sur onze vaches,
...
entre recouvrement et charge de fond~: c'est la cohérence entre familles, et non la seule
...
L'ablation soutient donc l'apport
de la direction du changement, de la cohérence entre signaux et de la persistance. Elle ne
résout pas la charge opérationnelle, qui reste supérieure à celle des variantes B et D.
...
Le Tableau~\ref{tab:fusions_v3} compare les cinq stratégies sur 132 scénarios. La colonne
...
de douze heures. Le fond HYPO de 0,446 diffère donc du 0,402 de la validation principale,
...
de HYPO. La Figure~\ref{fig:v3_fusions} compare les cinq règles de fusion.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/v3_fusion_comparison.png}
  \caption{Comparaison des règles de fusion.}
  \label{fig:v3_fusions}
\end{figure}
...
instabilité--hypoactivité est alertée dans 90,9\,\% des cas. Le Tableau~\ref{tab:scenarios_v3}
détaille ces réponses par scénario, que la Figure~\ref{fig:v3_scenarios} représente
visuellement.

\begin{table}[H]
  \centering
  \caption{Réponse de la fusion hiérarchique par scénario.}
  \label{tab:scenarios_v3}
  \small
  \begin{tabular}{lrrr}
    \toprule
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/v3_scenario_results.png}
...
\section{Concordance observationnelle avec les scores SLS}
...
$\rho=0{,}504$ avec $p=0{,}066$. Chaque point de la Figure~\ref{fig:v3_sls} représente une
vache; l'effectif de trois animaux dans le groupe SLS~$\geq2$ impose une interprétation
prudente.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{figures/v3_sls_notifications.png}
...
charge future atteint environ 0,755 notification par vache-jour. Le résultat est donc
fragile et complètement confondu avec le traitement \textit{Exercise}; il ne constitue pas
une validation clinique.

\section{Inspection visuelle ciblée}

La Figure~\ref{fig:v3_manual_review} montre quatre situations de la vache 8081 après le gel
...
\begin{figure}[p]
  \centering
  \makebox[\textwidth][c]{%
    \includegraphics[width=1.10\textwidth]{figures/v3_manual_review.png}%
  }
  \caption{Inspection visuelle de scénarios représentatifs.}
  \label{fig:v3_manual_review}
\end{figure}
...
La validation principale établit que HYPO transforme des baisses graduelles injectées après
la période de référence en épisodes attribuables, mieux localisés que ceux des comparateurs IF
et LOF. Le résultat est renforcé par trois contrôles complémentaires: le contrôle bref est
rejeté, l'ablation favorise HYPO chez les onze vaches, et l'analyse OFAT montre que la
conclusion ne dépend pas d'un seul seuil isolé. Le comparateur pédométrique obtient une
localisation proche, mais recouvre moins d'événements et produit une charge de fond plus
élevée.

L'extension bidirectionnelle ajoute une information de surveillance et récupère les séquences
synthétiques. Son intérêt est donc descriptif et opérationnel: elle organise mieux les sorties
lorsqu'une instabilité précède une baisse. Elle n'améliore toutefois pas IoU20, augmente le
fond et reste vulnérable aux causes non locomotrices. Elle constitue donc une piste
exploratoire, non une preuve de supériorité ou de précurseur clinique.
