"""Campagne technique du détecteur bidirectionnel de mémoire final."""
...
from validation_hybrid.profiles import HYBRID_PROFILES, HybridSyntheticProfile
...
def inject_profile(
    ...
    *,
    ...
    scenario_index = list(HYBRID_PROFILES).index(scenario)
...
def run_campaign(
    ...
    *,
    ...
    scenarios: Sequence[str] = tuple(HYBRID_PROFILES),
    ...
    if verbose:
        print(f"Campagne final terminée: {len(out)} événements sur {out['cow'].nunique() if len(out) else 0} vaches.")
    return out
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="data/validation/hybrid_initial")
