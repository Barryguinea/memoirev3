...
from revalidation.analysis import background_notification_rate
from revalidation.qa import write_json, write_sha256_manifest
...
def _git_diff_sha256(root: Path) -> str:
    diff = subprocess.check_output(
        ["git", "diff", "--binary", "HEAD", "--", "memoirev2"],
        cwd=root.parent,
    )
...
def finalize(root: Path, executed_notebook: Path) -> tuple[Path, Path]:
    ...
    snapshot = {
        ...
        "memoirev2_diff_sha256": _git_diff_sha256(root),
    ...
    provenance_paths = [
        ...
        root / "notebooks" / "revalidation_complete.ipynb",
        executed_notebook,
    ]
    for pattern in (
        "core/*.py",
...
def main() -> None:
    ...
    parser.add_argument(
        "--notebook",
        default="notebooks/revalidation_complete_executed_2026-06-12.ipynb",
    )
